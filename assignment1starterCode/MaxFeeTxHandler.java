...
public class MaxFeeTxHandler {
    ...
    public boolean isValidTx(Transaction tx) {
        ...
        for (int i = 0; i < tx.numInputs(); i++) {
            ...
            if (unique.contains(utxo)) {
                return false;
            }
            unique.addUTXO(utxo, output);
    ...
    public Transaction[] handleTxs(Transaction[] possibleTxs) {
        Set<Transaction> sorted = new TreeSet<>((tx1, tx2) -> {
            double tx1Fees = calcTxFees(tx1);
            double tx2Fees = calcTxFees(tx2);
            return Double.compare(tx2Fees, tx1Fees);
        });

        Collections.addAll(sorted, possibleTxs);
